    ...
    with input_path.open("r", encoding="utf-8") as infile, output_path.open(
        ...
        for line in infile:
            ...
            session = SessionContext(
                client_id=record.get("client_id"), product_id=record.get("product_id")
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--input", type=Path, required=True, help="Path to JSONL with queries."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("labeled_rag.jsonl"), help="Output JSONL."
    )
    parser.add_argument("--top-k", type=int, default=3)
    args = parser.parse_args()
    asyncio.get_event_loop().run_until_complete(
        label_rag(args.input, args.output, top_k=args.top_k)
    )
